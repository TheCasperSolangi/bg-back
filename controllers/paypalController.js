...
// Helper function to get vendor-specific PayPal configuration
const getVendorPayPalConfig = async (vendorCode) => {
  if (!vendorCode) {
  ...
  if (!storeSettings.paypal_client_id || !storeSettings.paypal_client_secret || !storeSettings.paypal_mode) {
    throw new Error(`PayPal credentials not configured for vendor: ${vendorCode}`);
  }
...
// Helper function to configure PayPal for vendor
const configurePayPalForVendor = async (vendorCode) => {
  const config = await getVendorPayPalConfig(vendorCode);
  
  // Create a new PayPal configuration for this vendor
...
exports.createPayment = asyncHandler(async (req, res) => {
  ...
  try {
    // Configure PayPal for this vendor
    await configurePayPalForVendor(vendorCode);
    ...
    const paymentObject = {
      ...
      redirect_urls: {
        return_url: `${return_url}?order_code=${order_code}&cart_code=${cart_code}&vendor_code=${vendorCode}`,
        cancel_url: `${cancel_url}?order_code=${order_code}&cart_code=${cart_code}&vendor_code=${vendorCode}`
      },
      transactions: [{
        amount: {
          currency: 'USD',
        ...
        description: `Order ${order_code} - ${cart.products.length} item(s) from Cart ${cart_code} (Vendor: ${vendorCode})`,
    ...
    paypal.payment.create(paymentObject, (error, payment) => {
      if (error) {
        console.error(`PayPal Payment Creation Error for vendor ${vendorCode}:`, error);
    ...
    res.status(500).json({
      success: false,
      message: 'Failed to configure PayPal for vendor',
      error: error.message
    });
  }
});
...
exports.executePayment = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    paypal.payment.execute(payment_id, executePaymentObject, async (error, payment) => {
      if (error) {
        console.error(`PayPal Payment Execution Error for vendor ${vendorCode}:`, error);
        return res.status(500).json({
          success: false,
      ...
      try {
        ...
        if (global.pendingOrders && global.pendingOrders[order_code]) {
          ...
          if (orderData.vendor_code !== vendorCode) {
            return res.status(400).json({
              success: false,
              message: 'Vendor code mismatch'
            });
          }
          ...
          // Verify vendor code matches
          if (minimalData.vc !== vendorCode) {
            return res.status(400).json({
              success: false,
              message: 'Vendor code mismatch in payment data'
            });
          }
        ...
        const order = await Order.create({
          ...
          vendor_code: orderData.vendor_code,
          user: {
...
exports.getPaymentStatus = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    paypal.payment.get(paymentId, (error, payment) => {
      if (error) {
        console.error(`PayPal Get Payment Error for vendor ${vendorCode}:`, error);
        return res.status(500).json({
          success: false,
      ...
      Order.findOne({ paypal_payment_id: paymentId, vendor_code: vendorCode })
        .then(order => {
          res.json({
            success: true,
            data: {
...
exports.refundPayment = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    // Find the order
    const order = await Order.findOne({ order_code, vendor_code: vendorCode });
    if (!order) {
      return res.status(404).json({
        success: false,
    ...
    paypal.sale.refund(saleId, refundData, async (error, refund) => {
      if (error) {
        console.error(`PayPal Refund Error for vendor ${vendorCode}:`, error);
        return res.status(500).json({
          success: false,
